...
import './pay.scss'
var configPayUrl = {
    url: "https://api.ucuxin.com/",//测试
    // url: "http://api.ucuxin.com/",//正式
    //payHrefUrl: 'http://pay.ucuxin.com/pay/thirdPartyPayment',//测试--文件的引入
    ...
    RedirectUri: 'http://pay.ucuxin.com/pay/getwx.html',//正式--微信支付完成的前置接口
...
var Pay_Party_Payment = {
  ...
  userInformation: {},
    ...
    directPay:function (params) {
      ...
      if(params.Channel == 11){
          Pay_Party_Payment.service.getWxOauth(11)
      }else if(params.Channel == 10){
          Pay_Party_Payment.service.CreatePayOrderwx(params);
      }else {
          Pay_Party_Payment.service.beforeGenerate(params.Channel);
      }
    },
    //选择支付方式
    selectPayType: function () {
        ...
        $('#weiXinApp').off('click').on('click', function () {
            Pay_Party_Payment.systemJudge() ? Pay_Party_Payment.service.beforeGenerate(11) : Pay_Party_Payment.service.getWxOauth(10);
        });
        //支付宝支付
    ...
    service: (function () {
        return {
            ...
            // 微信原生支付创建订单
            CreatePayOrderwx:function (params) {
                window.window.location.href = 'ucux://pay?uxpayid=' + params.UxPay.UxPayID;
            },
            //支付宝和银联支付
